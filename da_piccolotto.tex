...
Die Nutzerstudie wurde mit fünf Teilnehmern zwischen 23 und 25~Jahren durchgeführt (Tabelle~\ref{table:mockup:teilnehmer}). Dabei waren 60~\% der Personen männlich. Bei der Auswahl der Probanden wurde darauf geachtet, dass auch andere Domänen außer Informatik vertreten sind.
...
\begin{table}[htbp]
    ...
    \hline
    Teilnehmer 1 & W          & 23    & Zahnmedizin \\ %anna
    Teilnehmer 2 & M          & 25    & Informatik  \\ %fabi
    Teilnehmer 3 & M          & 23    & Informatik  \\ %kirsche
    Teilnehmer 4 & W          & 25    & Zahnmedizin \\ %karo
    Teilnehmer 5 & M          & 25    & Informatik  \\ %sebastian
    \end{tabular}
...
Nur einer der Teilnehmer konnte sofort erkennen, dass die vier Panels der Bedienungshilfe einen Comic und dieser wiederum eine Anleitung darstellte. Das sollte zusätzlich mit Text beschrieben werden. Außerdem könnte ein ansprechenderes Layout der Panels dazu führen, dass sie eher als Comic und weniger als vier unabhängige Bilder wahrgenommen werden. Einer der Teilnehmer merkte zusätzlich an, dass nicht ganz klar ist, ob der durch die Pfeile der statischen Kommunikationshilfe angedeutete Nachrichtenaustausch immer, nacheinander oder gleichzeitig stattfindet. Nachdem das für das Hilfesystem auch nicht eindeutig ist, kann in dieser Hinsicht leider nicht viel getan werden. Alle Teilnehmer waren sich einig, dass die letzten drei Panels des Comics auch ausreichen würden, da die Information in den ersten beiden redundant ist: Durch die Auswahl des Pfeils ist klar, um welche Komponenten es geht.

Es gab keinen eindeutigen Sieger bezüglich der Frage, ob die Kommunikationshilfe statisch oder dynamisch angeboten werden soll. Zwei bevorzugten die statische Variante, zwei fanden beide in Ordnung. Diese Anmerkungen kamen allerdings nach einer sehr übersichtlichen Situation im Mashup. In einer zusätzlichen Nutzerstudie sollten Grenzwerte für die Anzahl an Komponenten und Kommunikationskanälen bestimmt werden, bei denen die Kommunikation nicht mehr ohne weiteres nachvollzogen werden kann. Das Hilfesystem kann dann beim Start der kompositen Anwendung entscheiden, ob zusätzlich die dynamische Kommunikationshilfe eingesetzt werden muss.

Den Teilnehmern war nicht allein aufgrund der UI klar, dass Kommentare Annotationen enthalten können und wie diese hinzugefügt oder angezeigt werden. Die Kommentare sollten deswegen am Ende des Intros zusätzlich erklärt werden. Dasselbe Problem war bei der Bewertungsfunktion von Kommentaren zu beobachten. Die abstrakten, von StackOverflow\footnote{\url{http://stackoverflow.com}} inspirierten Icons wurden auch von den Informatikern teilweise mit \enquote{scrollen} anstatt \enquote{bewerten} assoziiert. Deswegen sollte noch einmal explizit mit Text beschrieben werden, dass sie eine Bewertungsfunktion darstellen und welche Semantik sich hinter ihnen verbirgt (\enquote{agree/disagree}). Eine weitere Erkenntnis daraus ist, dass Scrollmöglichkeiten immer explizit dargestellt werden sollten. Das kann entweder in Form einer Scrollbar oder wie im Metro User Interface von Windows Phone~7 mit angeschnittenen Interaktionselementen passieren.

Von zwei Teilnehmern kam der Vorschlag, die Anzahl der Kommentare immer darzustellen und nicht über einen Button in der Titelleiste zu toggeln. Hier ist aber zu beachten, dass die Komponenten im Paper Mockup sehr einfach gehalten waren und sehr eingeschränktes Interaktionspotential boten. Bei einem großen Scatterplot oder Parallel Coordinates ist diese Variante nicht mehr praktikabel.
...
% TODO zwei buttons um neueste und höchst bewertetste anzuzeigen
...
Hinzu kommt, dass sowohl alle möglichen Elemente, also auch Polygone, Polylinien und so weiter als auch deren Transformationen (verschieben, rotieren, skalieren, verzerren etc.) korrekt skaliert werden müssen. Außerdem ist der Vorgang wahrscheinlich sehr langsam, da der Browser nach jedem Update eines layoutrelevanten CSS Attributs (wie Breite oder Höhe) das Layout neu berechnen muss (\enquote{Reflow}) und währenddessen das User Interface blockiert. Dennoch erscheint die Aufgabe zwar nicht einfach, aber lösbar.
...
Wie in \cite{Heer2008} besteht die State History aus Bildern der Komponenten mit einer Seitenlänge von mindestens 120~Pixel (Abbildung~\ref{figure:undo-graphical}). Nach einem Klick auf einen Zustand wird die Komposition in diesen zurückgesetzt.
