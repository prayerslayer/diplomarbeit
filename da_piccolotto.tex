...
Das könnte beispielsweise umgangen werden, indem eine Art \enquote{Toggle} Button in der Mitte der Bounding Box platziert wird, welche sich immer über der Mitte des Elements befindet. \textbf{Wie sag ich hier jetzt, dass direkte Manipulation cooler ist?}

Eine andere Möglichkeit ist eine Funktion in die API einer Komponente hinzuzufügen, welche zwei Koordinaten auf einen Boolean abbildet und dem Hilfesystem so mitteilt, ob die Maus über der Repräsentation eines Datenpunkts liegt oder nicht. Aufgrund des Ziels der \textit{Minimalität} sollte davon aber möglichst abgesehen werden.

\subsection{Bedienung und Kommunikation}

Highlighting Algorithmus:

\begin{enumerate}
	\item Dunkel eingefärbtes, teilweise transparentes \texttt{div} über Komponente legen.
	\item Ein SVG Element in dieses einfügen, da die relevanten UI Elemente SVG oder HTML Elemente sein können.
	\item Mittels in SMCDL angegebenen CSS Selektor alle für die Assistance relevanten UI Elemente finden und in das div kopieren, wenn sie HTML ELemente sind und ins SVG kopieren, wenn sie SVG Elemente sind. Positionierungstechnisch ist bei SVG das Problem schon gelöst, weil der ganze visuelle Zustand im Markup ist und mitkopiert wurde. Für HTML Elemente setze ich position absolute sowie left und top. Weiters wird für jeden Selektor im Original und in der Kopie eine ID vergeben, die als Datenattribut \texttt{data-vizboard-copy} gespeichert wird. Das ist notwendig, sodass beim Highlight über ein Element in der Liste der CSS Selektor auf die originale Komponente gefahren werden kann UND die Kopien im Overlay gefunden werden.
	\item 
\end{enumerate}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{images/implementierung-kommentare-backend-architektur.png}
