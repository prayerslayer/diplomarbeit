...
\mmtsubmissiondate{30. Oktober 2013}
...
% Welche teilziele? Erstmal gucken, was es so für infovis gibt, was man so über assistance wissen muss. und sonst so?
...
Der folgende Abschnitt besteht aus fünft Teilen. Zuerst wird die Aufgabenstellung in einem Szenario verdeutlicht (Abschnitt~\ref{section:standderforschung:szenario}). Daraus werden Anforderungen an das Hilfesystem abgeleitet (Abschnitt~\ref{section:standderforschung:anforderungsanalyse}) und danach die Grundlagen von semantischen Daten, Informationsvisualisierungen und User Assistance bzw. Sensemaking erläutert (Abschnitt~\ref{section:standderforschung:grundlagen}). Konzepte verwandter Arbeiten (Abschnitt~\ref{section:standderforschung:verwandte_arbeiten}) werden auf ihre Anwendbarkeit untersucht, bevor das Kapitel mit einer Zusammenfassung (Abschnitt~\ref{section:standderforschung:zusammenfassung}) beendet wird.
...
Aus dem Szenario (Kapitel~\ref{section:standderforschung:szenario}) lassen sich nun verschiedene Anforderungen an ein Hilfesystem für komposite Informationsvisualisierungssysteme ableiten. Diese sind in funktionale und nicht-funktionale Anforderungen unterteilt (Abschnitte~\ref{section:standderforschung:anforderungsanalyse:funktionale_anforderungen} und~\ref{section:standderforschung:anforderungsanalyse:nichtfunktionale_anforderungen}).
...
Dieser Abschnitt beschäftigt sich mit den zur Konzeption nötigen Grundlagen. Diese beinhalten die CRUISe Plattform (Abschnitt~\ref{section:standderforschung:grundlagen:cruise_vizboard}), semantische Datensätze (Abschnitt~\ref{section:standderforschung:grundlagen:semantische_daten}), Informationsvisualisierung (Abschnitt~\ref{section:standderforschung:grundlagen:informationsvisualisierung}) und User Assistance (Abschnitt~\ref{section:standderforschung:grundlagen:user_assistance}). 
...
\begin{itemize}
	...
	\item Object Propertys beschreiben Beziehungen zwischen Klassen und deren Instanzen, beispielsweise eine Person \textit{besitzt} ein Smartphone.
...
OWL erweitert wiederum RDFS um einige Konzepte. So können beispielsweise Axiome definiert werden, Propertys können als transitiv oder symmetrisch deklariert werden, es existieren Mengenoperationen und Kardinalitäten und Wertebereiche können eingeschränkt werden. OWL existiert in zwei Versionen, da in den meisten Fällen nicht der volle Funktionsumfang benötigt wird: OWL Lite ist für Anwendungen gedacht, die kaum mehr als eine Klassenhierarchie und Attribute benötigen. OWL-DL ist auf die Logik in der Ontologie fokussiert und wird vor allem für Reasoner eingesetzt, um selbstständig Schlüsse innerhalb des vorgegebenen formalen Systems ziehen zu können. OWL 2 wurde 2009 eingeführt, dieser Standard erweitert OWL zum Beispiel um asymmetrische, reflexive und disjunkte Propertys.
...
Yahoo Pipes\footnote{\url{http://pipes.yahoo.com/pipes/}} ist eine Mashup Plattform von Yahoo, wo der Benutzer verschiedene Webservices mit Hilfe von Operatoren kombinieren kann, indem er sie interaktiv verdrahtet.
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{images/konzeption-ueberblick.png}
...
\begin{enumerate}
	...
	\item die Abhängigkeiten zwischen Komponenten mit Comics erklärt werden, genau wie bei der Bedienung (Ab\-schnitt~\ref{section:konzeption:bedienung}).
\end{enumerate}
...
\begin{itemize}
	...
	\item Konzept, welches eine Aktion näher beschreibt (Verlinkung).
	\item Anzahl an Sekunden, die zur vollständigen Änderung des User Interfaces benötigt werden (Bedienung).
...
\begin{itemize}
	...
	\item Eine Methode, welche Testdaten in die Komponente lädt (Bedienung).
\end{itemize}
...
\begin{enumerate}
	...
	\item Kommentare in Visualisierungen (Abschnitt~\ref{section:konzeption:kommentare})
...
% kommentare
% zwecks erkenntnisaustausch und -gewinn
% kommentare sowohl an daten (einzelne ressourcen und bereiche der propertys) als auch ortsbezogen
% entwickler: muss visualisierungswurzelelement kennzeichnen und methoden zur transformation daten <-> koordinaten implementieren. ist von allem bestimmt der meiste aufwand.
...
% history
% undo/redo für alle! gratis!
% was muss der dev machen? jede operation sendet nachricht und die muss die aufrufende operation enthalten. ansonsten eigentlich nüscht. (noch mal nachsehen)
...
% viele hilfefunktionen, die selbst wieder erklärt werden müssen
% statisch: text, bilder
% dynamisch: trace based reasoning. das hilfesystem versucht mit machine learning (decision tree) auf basis der interaktionen eines nutzers rauszufinden, ob und welche hilfe angeboten werden soll
% dev muss nüscht machen dafür
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.85\textwidth]{images/implementierung-cruise-architektur.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.3\textwidth]{images/implementierung-ui-visualization.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.75\textwidth]{images/implementierung-ui-howto-step1.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{images/implementierung-ui-howto-step2.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{images/implementierung-ui-capview.png}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.75\textwidth]{images/implementierung-ui-communication.png}
...
Damit die Komponente während der Hilfegenerierung auch tatsächlich Daten anzeigt, muss der Komponentenentwickler in der SMCDL eine URL zu Testdaten angeben (Listing~\ref{code:smcdl-testdata}). Dafür wurde ein weiteres Element \texttt{testdata} hinzugefügt. Im Gegensatz zu \texttt{exampledata} sind sie nicht Beispieldaten einer Eventnachricht, sondern ein ganzer Datensatz, der in die Komponente geladen wird. Es kann entweder eine \texttt{url} zu Testdaten enthalten, welche vor der Hilfegenerierung abgerufen werden, oder aus einer Datensatz ID des DaRe (\texttt{darefileid}) und einer SPARQL Query (\texttt{mapping}) bestehen, welche die zu ladenden Daten definieren.
...
\begin{enumerate}
	...
	\item Die Testdaten der Komponente werden geladen, indem selbige aus der Datei \texttt{mcdl.json} über die Methode \texttt{loadTestdata} an die Komponente gegeben werden.
...
% prototypische implementierung der konzepte bedienungshilfe, kommunikationshilfe, kommentare.
...
% das hilfe frontend bekommt die meisten informationen über eine komponente auch aus der smcdl (weil die über die tsr zugänglich ist). wichtig dabei halt gerade welche capabilities vorhanden sind. wenn der benutzer die bedienungshilfe dann aufruft, wird eine anfrage ans core gesendet, welche die pfade zu den bildern zurückschickt. die werden dann zusammen mit den captions eingeblendet.
...
% probleme:
% einerseits die art und weise, wie aktionselemente hervorgehoben werden, welche absolut unperformant arbeitet und jetzt konzeptionell nicht so wichtig ist, dass man sich den aufwand spart.
% andererseits hab ich ein problem, wenn die bilder vom backend nicht quadratisch (wie die panels) sind. die werden nämlich so angezeigt, dass die breite das quadrat füllt und gegebenenfalls sieht man überhaupt nix mehr. lösung wäre eben ein ordentlicher layoutalgorithmus, der das format berücksichtigt
...
% bei der kommunikationshilfe wurde die statische variante implementiert und in den capview von edyra integriert, weil dieser schon verbindungen zwischen komponenten zeichnet. diese unterscheidet sich nicht großartig von der bedienungshilfe, außer das in den panels jeweils das ergebnis einer operation und in den captions ein generiertes label (select a movie) zu sehen ist.
...
% bei den kommentaren wurde zunächst das dare so erweitert, dass es kommentare in rdf speichert (weil halt schon eine rdf speicher infrastruktur da war) und eine rest api, welche die kommentare in json ausgibt. es wurden methoden implementiert um ein kommentar hinzuzufügen, abzurufen, zu bewerten und zu löschen.
...
% im hilfe frontend werden annotationen mit hilfe von svg elementen umgesetzt, auf denen der benutzer zeichnet. deren position und größe wird dann mit der von der komponente implementierten methoden in datenbereiche umgewandelt. wenn kommentare angezeigt werden sollen, werden diese datenbereiche wieder zurück in koordinaten konvertiert und gezeichnet.

Das Frontend der Kommentare (Abschnitt~\ref{section:implementierung:frontend:kommentare}) zeigt Annotationen mit Hilfe von SVG Elementen an. Deren korrekte Position und Größe wird durch die von der Komponente implementierten Methode \texttt{dataToCoordinates} bestimmt, welche Daten in Koordinaten konvertiert. Nachdem ein Kommentar inklusive Annotationen verfasst wurde, werden die betroffenen Datenbereiche mit \texttt{coordinatesToData} ausgelesen und das Kommentar ans Backend geschickt. Danach kann es in einer anderen Komponente wieder geladen und dargestellt werden.

% einschränkung bezüglich bounding box von datenpunkten und relative größenangaben von ortsbezogenen bereichskommentaren und unterschiedlicher größe der komponente.
...
\begin{enumerate}
	...
	\item Dem Komponentenentwickler soll die Möglichkeit gegeben werden, seine Komponenten schnell und einfach in das Hilfesystem zu integrieren.
...
Für die Nutzerstudie wurden 10~Teilnehmer im Starbucks in der Prager Straße in Dresden (Abbildung~\ref{figure:location}) zufällig ausgewählt, da laut Virzi \cite{Virzi1992} mit sieben schon 80~\% der dringendsten Usabilityprobleme gefunden werden können. Die Teilnehmer waren zwischen~18 und 53~Jahren alt (Durchschnitt 30.3~Jahre, Median 27.5~Jahre, Standardabweichung 10.84~Jahre) und zu 60~\% weiblich (siehe Tabelle~\ref{table:evaluation-teilnehmer}). Es gab keine Kriterien, welche die Teilnahme an der Studie verhindert hätten.
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=\textwidth]{images/evaluation-location.jpg}
...
Die Integration in die Komponente dauerte weitere 10~Minuten und benötigte 8~Zeilen (Listing~\ref{code:integration-gif}). Alles in allem wurden 43~Minuten für diese Hilfefunktion aufgewendet.
...
% evaluation des prototype, einmal bezüglich annahme der hilfefunktionen (abschnitt blla), einmal bezüglich implementierungsaufwand für entwickler (bla hr)
...
% nutzertudie mit 10 teilnehmern im starbucks, entschädigung 5 euro, fünf fragen mit mashup lösen, danach sus fragebogen
% ergebnis: einige fehlinterpretationen wurden aufgedeckt, hauptsächlich haben die mit kopien von originalen (ein weiterer grund, das nicht so umzusetzen) und den beschriftungen des operationspanels zu tun. hilfe nicht so wahnsinnig viel verwendet (komm nur 1x), weswegen dyn. hilfe noch wert wäre implementeirt zu werden. usability fragebogen durchwachsen, aber insgesamt gutes ergebnis
...
% vergleich einer von kommilitione gewählten individuellen hilfefunktion für simile timeline mit integration hilfesystem. invididuelle hilfe war ein gif, dauerte 43 min zu integrieren und 8 zeilen code. hilfesystem (hat ein andrer gemacht) dauerte 32 minuten und 0 zeilen code. geht also schneller, aber wichtig sind eigentlich die andern vorteile: flexibilität, bessere wartbarkeit, einheitliche begriffe/bezeichnungen.
...
% grundlagen, was hab ich gelernt
...
% was muss in der aktuellen impl angepasst werden

Der vorgestellte Prototyp kann noch weiter verbessert werden. Zunächst sollten bei der Hervorhebung von aktionsauslösenden UI Elementen keine aufwändigen Kopien mehr erstellt werden, sodass \enquote{Layout Thrashing} verhindert und die Hilfefunktion dementsprechend schneller angezeigt werden kann. Zudem müsste für die Comicpanels ein Layoutalgorithmus geschrieben werden, der sowohl \enquote{ansprechendes Layout} als auch \enquote{effiziente Platznutzung} optimieren kann. Danach müssten im Frontend sowie im Backend bei der Hilfegenerierung parallele und sequentielle Operationen berücksichtigt und Tastaturoperationen unterstützt werden. Dabei kann gleichzeitig die Operation \emph{Drag} so erweitert werden, dass der Entwickler das erwünschte Event Target angeben kann. Außerdem wäre es für Anwender praktisch, wenn sie ihre Kommentare editieren könnten.

% howtolistview layout thrashing
% parallele, sequentielle operationen, tastaturoperationen

% was muss vom konzept angepasst und umgesetzt werden
...
\lstset{language=JSON}
\begin{lstlisting}[caption={JSON Format eines Kommentars}, label=code:implementierung-format-json]
{
  "comment_id": "id",
  "user_id": "bob",
