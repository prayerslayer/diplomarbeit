...
Zunächst muss begründet werden, warum ein anderes Javascript Framework als das in CRUISe bereits eingesetzte ExtJS\footnote{\url{http://www.sencha.com/products/extjs}} gewählt wurde. Hier müssen zwei Einsatzzwecke unterschieden werden: Als MVC Framework, welches verschiedene Views und Models definiert und strukturiert, sowie als Bibliothek für generische DOM Operationen. Für letzteres wird jQuery eingesetzt, da ExtJS einen höheren Abstraktionsgrad vom DOM aufweist. In ExtJS denkt der Entwickler nicht in \texttt{div}s oder \texttt{span}s, sondern in generischen Komponenten wie Listen, Panels und Dialogen. Als MVC Framework wird Backbone unter anderem deswegen eingesetzt, weil in der Einführung des Konzepts (Abschnitt~\ref{section:konzeption:einfuehrung}) ein visueller Unterschied zwischen Hilfesystem und Komponenten gefordert wurde. CRUISe Komponenten verwenden bereits die generischen ExtJS Styles, weswegen das Frontend eigene Styles für das recht komplexe, von ExtJS generierte Markup definieren müsste. Dies erschien zu viel Aufwand zu sein. Von der Einheitlichkeit der Codebase abgesehen gab es auch keinen Grund, ExtJS zu verwenden, weil das User Interface des Frontends recht einfach aufgebaut ist.

% wenn schon kein extjs, warum gerade backbone und nicht knockback, spine oder eins von den andern tausend?
Warum ausgerechnet Backbone.js? Einerseits hab ich schon mal was damit gemacht, zweitens ist es sehr populär (siehe Backbone Website, wer es alles verwendet) und hat gute Dokumentation. Wegen Popularität findet man auch viel auf StackOverflow, was man auch nicht unterschätzen darf.

% marionettejs macht backbone code lesbarer und einfacher zu warten
Marionette.js erweitert Backbone um verschiedene Konzepte (z.\,B. Regionen und unterschiedliche Views) und automatisiert oft vorkommende Anweisungen (wie zum Beispiel einen View rendern und in den DOM einfügen). Der Code ist dadurch lesbarer und einfacher zu warten.

% und was hat d3 hier zu suchen?
D3 wird hauptsächlich beim Hinzufügen von Annotationen eingesetzt, wo ein SVG Element über die Komponente gelegt wird, in welchem der Benutzer seine Annotationen zeichnen kann. Außerdem kann D3 im Gegensatz zu jQuery sowohl mit HTML- als auch mit SVG Elementen umgehen. In jQuery funktionieren einige Methoden, die im Frontend gebraucht werden (z.\,B. \texttt{.width()}, \texttt{.addClass()} oder jQuery UIs \texttt{.position()}), nicht für DOM Elemente mit SVG Namespace. Zwar existiert SVG Plugin\footnote{\url{https://github.com/kbwood/svg}} für jQuery, dieses weist aber eine unnötig komplizierte API auf und unterstützt nur jQuery 1.7\footnote{Aktuelle Version zum Zeitpunkt der Entwicklung: 2.0.3.}. Deswegen wird für die Größenbestimmung oder Positionierung von SVG Elementen D3 verwendet, da es ohnehin schon eingesetzt wird.

\subsection{Kommentare}

Hier ist glaub ich nur der AddAnnotationView interessant, der Rest ist recht straightforward.

Im AddAnnotationView sind leider alle vier Werkzeuge zusammen anstatt 1 View pro Werkzeug. Grund: Alle müssen sichtbar sein (ausblenden einzelner Views nicht möglich), aufeinander abgestimmt (wie steht fest, welcher aktiv ist?) und funktionieren (kann zB nicht einzelne Views auf Gruppen arbeiten lassen, weil Gruppen keine pointer-events bekommen, wenn nix drin is).
...
\begin{itemize}
	...
	\item data-vizboard-copy Attribut um Selektor auf originale Komponente fahren und trotzdem die Kopien in vizboard-rootcopy finden zu können. Das ist wichtig für Selektoren wie nth-child-of(odd) oder "ancestor child". Ich kopier ja nur die Elemente, die von den Selektoren getroffen werden, nicht alles ringsrum.
\end{itemize}
...
Weiß nicht ob ich das behandeln soll. Hier kann ich maximal ein Klassendiagramm hinknallen und so komplex ist ja alles dann nicht. Ein CommentBackend für die Schnittstelle zu Jena (speichern/laden), ein CommentRequestHandler fürs HTTP Request Handling (GET/POST/PUT) und dazwischen eigentlich nur der CommentConverter, der zwischen Jena, Java und JSON konvertiert.
...
\begin{itemize}
	\item \texttt{GET dare/comment} gibt alle Kommentare aus. Sind \texttt{property} (für visualisierte Properties) oder \texttt{component} (für Komponenten) Parameter im Querystring vorhanden, wird nach diesen gefiltert und Kommentare gesucht, die mindestens ein Kriterium erfüllen.
	\item \texttt{GET dare/comment/\{id\}} gibt Kommentar mit ID \texttt{id} aus.
	\item \texttt{POST dare/comment} nimmt ein Kommentar in \texttt{application/json} entgegen und speichert es ab.
	\item \texttt{PUT dare/comment/\{id\}} nimmt ebenfalls ein Kommentar in JSON entgegen und aktualisiert die Daten des Kommentars mit ID \texttt{id}. Wenn etwas verändert wurde, antwortet der Server mit Status \texttt{200 OK}, ansonsten \texttt{304 Not Modified}. So ist sie wie von RFC 2616 \cite{W3C:rfc2616} gefordert idempotent: Kommen mehrere Male gleiche Daten an, verändert sich der Zustand des Kommentars im Backend nicht.
\end{itemize}
