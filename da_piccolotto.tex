...
Um automatisch Comicpanels zur User Assistance erstellen zu können, werden Aktivitäten zunächst nach Kuutti \cite{Kuuti1996} in Operationen, Aktionen und Aktivitäten unterteilt.
...
Multimodal (verbal/akustisch und visuell) wäre lt Grundlagen gut, aber von UX Standpunkt aus gesehen nervt plötzlicher Sound tierisch. Außerdem globales UI Ziel 1: Schnelles Verständnis klappt nicht so dolle mit Audio, weil das muss man fertig hören. Weiters Problem: Gleichzeitigkeit ist lt Grundlagen bei Multimedia nötig, und das kann bedeuten, dass die Animation aufs Audio warten muss oder umgekehrt, was wiederum nicht gut ist. Also Comic-Animation in 4 Panels (1: Überblick, 2: Wie 1, dann Zoom auf relevante Elemente, 3: Wie 2, dann Overlay mit Operation (Click, Drag, Hover...), 4: Wie 2, dann wieder rauszoomen und Unterschied sehen. Die Frage ist noch WER dafür verantwortlich ist: Entweder die MRE (Component Manager) oder wir erweitern das Default Ext Frame, in dem alle Komponenten gekapselt sind.


Wie sieht die UI aus? ?-Button in der Titelleiste. Dann Overlay über Komponente, interaktionsförderliche Elemente nicht davon betroffen. Oben liste mit Caps wozu es hilfe gibt (2way). Hover gibt Text "Klicken um Hilfe zu Zoom zu sehen". Klick macht richtiges Overlay wo Comicanimation abgespielt wird.
...
\begin{enumerate}
	\setcounter{enumi}{3}
	\item Die Komponente darf keine dynamischen Daten anzeigen, wie zum Beispiel die neuesten Tweets eines Twitter-Accounts.
...
In der Titelleiste gibt es einen Button mit einer Sprechblase. Dort draufgeklickt erscheint ein Popover mit 2 Buttons: Plus und Auge.

Plus ist hinzufügen, nach einem Klick wird das Popover mit einer Textbox gefüllt und der Viewport wird abgedunkelt. Man kann nur Rechtecke auf den Viewport hinzufügen, weil so am wenigsten Arbeit für den Dev entsteht. Dafür aber mehr als eins. Oder man wählt Datenpunkte aus. (Frage: Wie dieselbe Interaktion für beide Kommentartypen haben?) Ist man fertig mit dem ganzen, holt die MRE zusätzliche Daten - falls möglich, siehe Treemap - von der Komponente, sendet das ans DaRe zum speichern und setzt die Komponente in AUsgangszustand zurück.

Auge ist anzeigen, nach einem Klick wird eine Liste von Kommentaren im Popover angezeigt. Die kann man auswählen, dann wird der Bereich - falls vorhanden - wieder reingeladen (Vorsicht! Komponente ist interaktiv, zuvor muss also der Zustand zum Zeitpunkt des Kommentars wiederhergestellt werden --> Memento definieren). Jeder Kommentar enthält Text, Bild, Avatar, Benutzernamen, Datum/Uhrzeit, evtl Buttons zum Ändern/Löschen falls eigene Kommentare. Außerdem Anzahl KOmmentare an jeden Datenpunkt überblenden so wie NOtification iCons. Hier sind Kommentare anderer Vis sichtbar.
