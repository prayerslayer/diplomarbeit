...
Animationen können ebenfalls ein geeigneter Weg sein um Wissen zu vermitteln, sofern die erwähnten Designprinzipien eingehalten werden \cite{Mayer2002a}. Allerdings ist es möglich, dass Benutzer mit wenig Vorwissen weniger davon profitieren also solche mit viel Vorwissen \cite{Kayuga2008}. Wenn Animationen eingesetzt werden, sollte den Benutzern Kontrolle über die Geschwindigkeit der Animation gegeben und wichtige Teile gekennzeichnet werden \cite{Wong2011}.

Filme? Hier gibt es eine Doktorarbeit, aber die finde ich online nicht \url{http://search.proquest.com/docview/1286857518}. Ansonsten scheint sich damit niemand so richtig zu beschäftigen. Vielleicht kann ich Semantic Transparency hier anbringen.
...
\cite{Russell1993}: Learnining Loop. Im Prinzip ist es dasselbe wie Klein2006 oder der Visual Analytics Process.

\cite{Klein2006a}: Im Prinzip hat man ein mentales Modell, wo versucht wird, die Daten reinzupassen. Gegebenenfalls werden entweder die Daten oder das mentale Modell in Frage gestellt. Das ist ein iterativer Prozess, der nicht wirklich einen Anfang oder ein Ende hat.

\cite{Klein2006}: Was nicht zwingend hilft beim Sensemaking: Zuviel Datenvorverarbeitung, zuviel präsentierte Daten, Hypothesen die nicht überprüfbar sind.

\cite{Fisher2012, Qu2005, Novak2007, Umapathy2010} betonen die Wichtigkeit von der Struktur der Wissensrepräsentation. Wissensexternalisierung ist aber wichtig und hilft beim Verständnis. Aber eben Struktur > Inhalt.

\cite{Grammel2012}: Matching the user's mental model ist wichtig. (seite 127!)

\cite{Kohlhase2009} muss auch irgendwie rein. Wichtige Eigenschaft von UI.
