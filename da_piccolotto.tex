...
Ein weiteres Problem der vorgestellten und implementierten Bedienungshilfe ist Performance. War das Markup der Komponente tief verschachtelt, dauerte die Generierung der Kopien der relevanten UI Elemente bis zu 15 Sekunden. Das war bei der ExtJS Tabelle mit Testdaten von 60~Filmen in 6~Spalten der Fall, mit größeren Datensätzen würde es noch viel länger dauern. Nach einer Inspektion mit den Chrome Developer Tools stellte sich heraus, dass eine Kombination aus exzessivem \enquote{Layout Thrashing} und häufiger Garbage Collection das Problem war. Das Layout Thrashing hatte folgende Auslöser: Zunächst musste für jedes der relevanten Elemente die sichtbare Bounding Box berechnet werden, wofür Zugriff auf die Offsets nötig ist. Dafür wird ein aktuelles Layout benötigt, der Browser muss es also berechnen. Danach wird die Kopie in den DOM eingefügt, was das Layout invalidiert und es somit beim nächsten Element erneut berechnet werden muss. Zum Teil kann der Algorithmus bestimmt optimiert werden, aber nachdem Datensätze in VizBoard beliebig groß und Komponenten in CRUISe beliebig verschachtelt sein können, sollte auf diese Art der Darstellung verzichtet werden. Stattdessen sollten relevante UI Elemente in ihrer normalen Umgebung hervorgehoben werden, ohne aufwändige Erstellung von Kopien.
...
Für die Integration der Komponente in das vorgestellte Hilfekonzept müssen zunächst die notwendigen Informationen in die SMCDL Datei eingefügt werden. Dabei ist hauptsächlich die Beschreibung der Capabilities relevant (Listing~\ref{code:integration-capability}). Der Tippaufwand beträgt 5~Minuten, für die anderen Annotationen werden noch einmal 5~Minuten veranschlagt. Die SMCDL ist danach um 40~Zeilen länger.

Falls die Komponente auch Kommentare unterstützen soll, müssen danach das Wurzelelement der Visualisierung sowie die enthaltenen Ressourcen mit einem Datenattribut versehen werden. In den meisten Fällen ist das eine zusätzliche Zeile Sourcecode.
