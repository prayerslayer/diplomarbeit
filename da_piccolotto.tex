...
Über die Interaktion mit der Visualisierung wird hier noch keine Aussage gemacht, da sie sehr umfangreich sein kann und den Rahmen des Überblicks sprengen würde. In Abschnitt~\ref{section:konzeption:bedienung} wird eine Hilfestellung zur Bedienung einer Komponente erarbeitet.
...
Am einfachsten würde diese umgesetzt, indem dem Komponentenentwickler eine Möglichkeit gegeben wird, Text und Bilder zu diesem Thema in die Komponentenbeschreibung einzufügen. Das hat allerdings den Nachteil, dass das Hilfesystem keinen Einfluss auf die Wortwahl, Aussehen der Bilder oder Ausführlichkeit der Erklärung hat. Wie in den Grundlagen (Abschnitt~\ref{section:standderforschung:grundlagen:user_assistance}) gezeigt, können Comics außerdem eine effektivere Lösung als Text für User Assistance sein. Allerdings stellt es für den Entwickler erheblichen Mehraufwand dar, für jede mögliche Aktion verschiedene Comicpanels zu erstellen. Davon abgesehen können (und sollen) die gestalterischen Fähigkeiten dazu nicht von ihm erwartet werden und das Problem der Kontrolle über den Inhalt ist auch nicht gelöst.
...
Um zu erklärende Konzepte für das Hilfesystem auffindbar zu machen, zeichnet der Komponentenentwickler sie mit einer dafür vorgesehenen CSS Klasse aus. So können sie über CSS Selektoren gefunden werden. Betreffen die Konzepte Achsen der Visualisierung, ist das korrespondierende DBpedia\footnote{\url{http://dbpedia.org/}} Konzept bereits im Domain Assignment enthalten, welches die MRE der Komponente mitteilt. Für frei gewählte Worte müsste eine eigene Anfrage an das Data Repository gestellt werden, welches das entsprechende Konzept der DBpedia findet, das Abstract lädt und zurück liefert. Dann kann es zusammen mit einem Link zum Wikipedia-Artikel angezeigt werden.
...
\begin{figure}[htbp]
   ...
   \caption{UI Mockup: Comic zur Kommunikation}
...
\begin{itemize}
	...
	\item Rechte Spalte
	\begin{itemize}
		\item Voting: Ein Benutzer kann einen fremden Kommentar genau bewerten. Die Anzahl der Stimmen wird ebenfalls angezeigt.
...
\begin{itemize}
	\item VISO Konzept der grafischen Repräsentation (Überblick)
...
\begin{itemize}
	\item Beschreibung einer grafischen Repräsentation (Überblick)
	\item Link zu Tutorialvideo (Überblick) -- gehört sowas wirklich in die VISO?
\end{itemize}

\section{Beispiel}
\label{section:konzeption:beispiel}

Vielleicht sollte ich ein oder zwei InfoVis aus \cite{Heer2010} exemplarisch durchgehen um zu beweisen, dass das mein Konzept auch funktioniert? Ansonsten müsste man mir das an dieser Stelle einfach glauben, bevor es zur Implementierung geht. Oder wäre es sowieso geschickter, wenn ich das am Ende der Implementierung mache?
