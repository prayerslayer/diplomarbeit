...
Im Fall von Interaktionselementen wird bei \texttt{MouseOver} ein Hilfebutton eingeblendet, der den selben Effekt hat (Abbildung~\ref{figure:verlinkung-bedienung}). Das hat den Grund, dass Interaktionselemente in der Regeln nicht nur \texttt{div}, \texttt{svg:text} oder \texttt{span} Elemente sind, sondern auch z.\,B. \texttt{input} unterschiedlichen Typs sein können. So kann eine einheitliche Interaktionsgestaltung unabhängig des Elements gewährleistet werden.
...
In Abschnitt~\ref{section:konzeption:kommentare:api} wurde ein Memento-Pattern für Komponenten eingeführt. Damit kann das Hilfesystem Zustände von Komponenten abrufen, speichern und wiederherstellen. Es stellt sich die Frage, wann der Zustand aller Komponenten abgerufen werden soll. Nach jedem Event erscheint ausreichend, da Properties den visuellen Zustand der Komponente vollständig beschreiben sollen und falls sich dieser ändert, ein \texttt{propertyChanged} Event gefeuert wird. Das Hilfesystem ruft dann den Zustand aller Komponenten ab und speichert ihn in einen Stack. Im Falle eines Undo wird der oberste Anwendungszustand vom Stack genommen und jede Komponente wiederhergestellt. 
...
\begin{enumerate}
	...
	\item Das Hilfesystem schickt die erfassten Daten an die Wissensbasis.
	\item Die Wissensbasis erlernt Regeln aus den Daten, beispielsweise männliche Architekten brauchen in der Nacht Hilfe zur Kommunikation, wenn sie im Schnitt länger als 10~Sekunden zwischen Interaktionen benötigen.
