...
\textbf{Der Inhalt der Visualisierung entspricht dem Domain Assignment, das muss ich wohl noch mal nachlesen wie das genau war.}
...
Um mögliche Aktionen des Benutzers zu klassifizieren, wird die hierarchische Aktivitätstheorie nach Kuutti \cite{Kuutti1996} zurückgegriffen:
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Für jeden definierten CSS Selektor tue folgendes:
		\begin{enumerate}
		...
		\end{enumerate}
		\item Führe die definierte Operation auf einem beliebigen relevanten Element aus
...
Todo: Noch erklären, warum DBpedia und nicht z.B. urbandictionary?
...
\subsection{Unterstütze Funktionen}
\label{section:konzeption:kommentare:funktionen}
...
Erfahrungsgemäß tragen nicht alle Kommentare gleich viel zur Diskussion bei. Man denke dabei an Hinweise auf Tippfehler oder das berüchtigte \enquote{Erster!}. Deswegen ist es notwendig, dass Benutzer Kommentare anderer bewerten können\footnote{Ob dabei der Inhalt bewertet wird, steht wiederum auf einem anderen Blatt.}. Das Hilfesystem kann dann Kommentare nach den meisten Upvotes sortiert anzeigen.
...
Todo: Kann ich hier von Idealfällen reden oder muss ich mich für JA/NEIN entscheiden? Sicher wäre es klasse, aber ich hab grad überhaupt keine Lust, mir UI und Backend für eine Versionsverwaltung von Kommentaren zu überlegen.
...
\begin{itemize}
	...
	\item Kommentare können gelöscht werden, solange keine Referenz darauf besteht. Das sieht aus als hätte ich es nicht hinbekommen. Lauwarmer Kompromiss, jawohl.
	\item Kommentare können beliebig gelöscht werden. Diese Variante weckt keine Ängste, den eigenen Stuss nicht mehr löschen zu können. Dann muss aber etwas mit den Referenzen auf gelöschte Kommentare geschehen, da ansonsten die Nachvollziehbarkeit der Diskussion leidet. Beispielsweise könnte die referenzierte Originalversion abgespeichert und eingeblendet werden.
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Avatar des Benutzers und Benutzername sollen das Vertrauen der Benutzer untereinander erhöhen und so die Qualität der Kommentare erhöhen \cite{Chiu2006}.
	...
	\begin{itemize}
		\item Kommentartext mit Referenzen (durch \#hash gekennzeichnet) und URLs (unterstrichen).
	\end{itemize}
	\item Rechte Spalte
	\begin{itemize}
		\item Voting: Ein Benutzer kann einen fremden Kommentar genau einmal up- oder downvoten. Die Anzahl der Scores wird ebenfalls angezeigt.
...
Den Ausführungen in den Abschnitten~\ref{section:konzeption:kommentare:api} und~\ref{section:konzeption:kommentare:funktionen} nach enthält ein abgespeichertes Kommentar im DaRe folgende Daten:
...
Die Alternative zum statischen Button wäre dem Benutzer dynamisch und kontextabhängig Hilfe anzubieten, beispielsweise in Form von Notifications wie in OSX. So könnten ihm Erklärungen zu den Kommentaren angeboten werden, sobald er auf den Kommentar-Button geklickt hat oder eine allgemeine Hilfe, wenn er längere Zeit keine Aktion ausgeführt hat. Der große Vorteil daran ist, dass nur die Benutzer die Meta-Hilfe sehen, welche sie auch benötigen. Allerdings hat diese Lösung einige Nachteile. Zunächst ist es so, dass Notifications nach einer festgelegten Zeitspanne wieder verschwinden müssen, da sie sonst stören. Dadurch ist es dem Benutzer unmöglich, aus eigener Initiative zur Meta-Hilfe zu gelangen. Hinzu kommt, dass die Meta-Hilfe für die meisten Funktionen recht kurz ausfällt und nur bei Kommentaren und der Bedienung unbedingt notwendig ist. Wenn die Hinweise auf die Meta-Hilfe aber nur bei zwei von sechs Funktionen erscheinen, ist das zu inkonsistent und wirkt eher irritierend. Davon abgesehen müssen für die kontextabhängige Hilfe einige Probleme bewältigt werden, wenn sie wie gewünscht funktionieren soll. Das Hilfesystem muss Regeln kennen, wann Hilfe angeboten werden soll. Die Reaktionen des Benutzers darauf sollen bewertet werden und zusammen mit dem Anwendungskontext wieder in die Wissensbasis einfließen, damit das System daraus lernen und neue Regeln inferieren kann. Das ist ganz schön viel Arbeit dafür, dass die Meta-Hilfe in dieser Arbeit nicht im Vordergrund steht. Deswegen gehen wir mit dem statischen Button. So. In Zukunft kann das aber auch zusammen verwendet werden, kein Problem.

\subsection{User Interface \& Interaktion}

Der Button für die Meta-Hilfe befindet sich in der rechten oberen Ecke des Viewports (Abbildung~\ref{figure:meta-step1}). Er ist rot, damit er vom Benutzer bemerkt wird und rund, weil er so aus dem bisher verwendeten optischen Konzept ausbricht, aber nicht fehl am Platz wirkt.
...
Nachdem der Benutzer darauf geklickt hat, wird der Viewport abgedunkelt und ein Fenster überblendet, welches Hilfe zu verschiedenen Themen enthält (Abbildung~\ref{figure:meta-step2}).
...
\subsection{Markup einer Komponente}

\begin{itemize}
	\item CSS Klasse für automatische Verlinkungen
	\item CSS Klasse für Wurzelelement der Visualisierung (Kommentare)
	\item Datenattribut mit URI für Datenpunkte (Kommentare)
\end{itemize}

\subsection{Komponentenbeschreibung}

\begin{itemize}
	\item Graf. Repräsentation (Überblick)
	\item CSS Selektoren zu Capabilities (Bedienung)
	\item Operationen zu Capabilities (Bedienung)
	\item Anzahl Sekunden zu warten vor Screenshot (Bedienung)
	\item Boolean ob Hilfe für Capability/Operation generiert werden soll oder nicht (Bedienung)
	\item Beispieldaten für Operation, falls sie Parameter erwartet (Bedienung)
	\item Memento (Kommentare)
	\item Boolean ob graf. Rep. die Position kodiert (Kommentare)
	\item Boolean ob Komponente Datenverarbeitung durchführt (Kommentare)
	\item Boolean ob externe Daten angezeigt werden (Kommentare)
	\item Boolean ob Layout vollständig reproduzierbar ist (Kommentare)
\end{itemize}

\subsection{API einer Komponente}

\begin{itemize}
	\item Funktion, die Koordinaten in Datenpunkte umrechnet (Kommentare)
	\item Deren Umkehrung (Kommentare)
\end{itemize}

\subsection{VISO}

\begin{itemize}
	\item Beschreibung einer graf. Repräsentation (Überblick)
	\item Link zu Tutorialvideo (Überblick)
\end{itemize}
