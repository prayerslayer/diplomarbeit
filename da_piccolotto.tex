...
Bei der Generierung der Panels ist zuerst das Component Repository beteiligt. Sobald eine Komponente hinzugefügt oder upgedatet wurde, parst das CoRe die entsprechende Komponentenbeschreibung und schickt eine Anfrage an den Hilfeservice. Dieser kann ins CoRe integriert sein, sollte dem \enquote{Separation of Concerns} Designprinzip folgend aber davon getrennt sein, da es schon für Matching, Ranking und Verwaltung der Komponenten zuständig ist. Der Hilfeservice rendert die Komponente mit Hilfe des Headless Browsers in einer CRUISe Testumgebung. 

% todo cruise testumgebung, was gehört da alles rein?
% todo cruise testumgebung, wo kommen vor allen dingen die daten her?

% möglicher datenursprung:
% komponentenentwickler: gibt url an oder hängt datei an beim hochladen, von wo dare die daten holt. vorteil: notwendige daten existieren auf alle fälle. nachteil: aufwand. obwohl man eigentlich davon ausgehen kann, dass es im laufe der entwicklung der komponente IRGENDWELCHE testdatensätze mal gegeben haben wird.
% vizboardbetreiber: stellt testdatensätze zur verfügung, die entwickler auswählen kann. vorteil: geringer aufwand für entwickler. nachteil: spezielle daten evtl nicht vorhanden.

Danach führt es folgende Schritte aus:
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Für jeden definierten CSS Selektor:
		\begin{enumerate}
